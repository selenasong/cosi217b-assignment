...
class Entities(db.Model):
   ...
   token = db.Column(db.String, primary_key=True)
   entity_count = db.Column(db.Integer, primary_key=False, default=1)
...
def add_relations_to_db(dependency_parse, entities):
    with app.app_context():
        ...
        for _, _, _, text in entities:
            existing_entity = Entities.query.filter_by(entity=text).all()
            if existing_entity:
                for entity_token in existing_entity:
                    entity_token.entity_count += 1
            else:
...
def show_all():
    entity_relations = Token_relation.query.join(Entities, Entities.token == Token_relation.token).add_columns(
         Entities.entity, Entities.entity_count, Token_relation.relation, Token_relation.count).all()

    return render_template('show_all.html', entity_relations=entity_relations)
